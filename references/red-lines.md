...
**Action:** Reject the affected section and rewrite with functional equivalence. The test: would a native speaker of the target language write this from scratch?
...
**Action:** Flag each dependency. Replace with export-legible equivalents using the social logic rewrite table in `adaptation-layers.md`. If no clean replacement exists, restructure the scene motivation.
...
**Action:** Flag the format-tone mismatch. Either adjust the script tone toward realism OR recommend a stylized format (2D/anime) that embraces the exaggeration.
...
**Action:** Recommend stylization instead. If 3D is mandatory, flag specific scenes where uncanny valley will be most damaging and propose mitigation (camera angles, lighting, reduced closeups).
...
**Action:** Flag the specific sponsor-toxic elements. Propose safer reframing that preserves the emotional engine while removing sponsor-incompatible content. If the premise is fundamentally sponsor-unsafe, document the monetization risk explicitly.
